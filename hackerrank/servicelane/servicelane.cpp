...
vector<int> serviceLane(vector<int> width, int n, vector<vector<int>> cases) {
    ...
    for (auto v : cases) {
        int entrance;
        int exit;
        int maxvehiclewidth = 100; // Max lane width is 3, so 100 is bigger
        int i;
        entrance = v[0];
        exit = v[1];
        for (i = entrance ; i <= exit ; i++) {
            if (width[i] < maxvehiclewidth) {
                maxvehiclewidth = width[i];
            }
        }
        result.push_back(maxvehiclewidth);
    }
    return result;
